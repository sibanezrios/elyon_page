...
export function FeatureGrid({ type }: FeatureGridProps) {
  ...
  return (
    <motion.div
      ...
      className={`grid grid-cols-1 md:grid-cols-2 ${type === 'services' ? 'lg:grid-cols-4' : 'lg:grid-cols-3'} gap-6 md:gap-8`}
    >
      {data.map((feature, index) => (
        <motion.div
          key={index}
          variants={item}
          className="bg-black/30 p-6 rounded-xl shadow-lg hover:shadow-2xl hover:shadow-brand-500/20 transition-all border border-brand-500/30 hover:border-brand-500/60 backdrop-blur-sm"
        >
